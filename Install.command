...
printf '\n'
red "Instalacja nie została ukończona, ale poprzednia wersja i poczta pozostały bezpieczne."
if [[ -s "${STATUS_FILE}" ]]; then
  while IFS= read -r line; do red "${line}"; done < "${STATUS_FILE}"
fi
printf '%s\n' "Najpierw uruchom ponownie Install.command. Jeśli błąd się powtórzy, przekaż poniższy plik osobie pomagającej." >&2
